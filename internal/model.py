...
class Sale(db.Model):
    ...
    addons = db.relationship("AddOns", backref='sale')
    ...
    def get_subtotal_sum(self):
        ...
        sub_sum = 0
        for item in self.items:
            sub_sum += item.calculate_subtotal()
        
        return sub_sum
    
    def get_addon_sum(self):
        """external - sum of all item total"""
    ...
    def get_broker_sum(self):
        """external - sum of all item total"""
        
        sub_sum = 0
        for staff in self.staff_relations:
            sub_sum += staff.broker_fee
    ...
    def get_receipt_total(self):
        ...
        return self.get_subtotal_sum() + self.get_addon_sum() 
    
    def outstanding_broker_fee(self):
        """internal - return total receipt amount to be charged to client"""
        
        fee_outstanding = 0
        
        for staff in self.staff_relations:
            if staff.broker_fee_paid == False:
                fee_outstanding += staff.broker_fee
...
class Item(db.Model):
    ...
    def __init__(self, intake_id, quantity, sale_price, sale_id, notes="N/A"):
        
        intake_instance = (Intake.query.filter(Intake.id == intake_id).first())
        product_id = intake_instance.product.id
        
        self.product_id, self.intake_id, self.quantity, self.sale_id, self.notes, self.sale_price, self.notes = (product_id, intake_id, quantity, sale_id, notes, sale_price, notes)
    ...
    def calculate_subtotal(self):
        ...
        sub_total = self.quantity * self.sale_price
...
class StaffRelations(db.Model):
    ...
    id = db.Column(db.Integer, autoincrement=True, primary_key=True)
    staff_id = db.Column(db.Integer(), db.ForeignKey(Staff.id))
    broker_fee = db.Column(db.Float(10), nullable=True)
    ...
    def __init__(self, staff_id, broker_fee, broker_fee_paid, intake_id=None, sale_id=None, sample_id=None, notes="N/A"):
        self.staff_id, self.broker_fee, self.broker_fee_paid, self.intake_id, self.sale_id, self.sample_id, self.notes = (staff_id, broker_fee, broker_fee_paid, intake_id, sale_id, sample_id, notes)
